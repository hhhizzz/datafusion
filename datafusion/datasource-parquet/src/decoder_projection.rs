...
impl DecoderProjection {
    ...
    /// Decompose the per-batch transform for stream drivers that own output
    /// projection independently from the decoder configuration.
    pub(crate) fn into_output_parts(self) -> (Projector, SchemaRef, bool) {
        (self.projector, self.output_schema, self.replace_schema)
    }
