...
pub(super) struct ParquetMorselizer {
    ...
    pub(crate) lookahead: Option<LookaheadScanContext>,
}
...
impl RowGroupsPrunedParquetOpen {
    ...
    fn build_stream(self) -> Result<BoxStream<'static, Result<RecordBatch>>> {
        ...
        let (decoder, rg_plan, prefetch_plan) = {
            ...
            (builder.build()?, rg_plan, prefetch_plan)
        };
        ...
        let stream = match (prepared.lookahead, row_group_pruner) {
            (Some(lookahead), None) => {
                let (projector, output_schema, replace_schema) =
                    decoder_projection.into_output_parts();
            ...
            (lookahead, row_group_pruner) => {
                drop((lookahead, prefetch_plan));
                PruningPushDecoderStreamState {
